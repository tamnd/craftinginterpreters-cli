...
describes what users get. Pick a format with `-o`, or let ci choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ci <command> -o table   # aligned columns for reading
ci <command> -o jsonl   # one JSON object per line, for piping
ci <command> -o json    # a single JSON array
ci <command> -o csv     # spreadsheet friendly
ci <command> -o tsv     # tab-separated
ci <command> -o url     # just the URL column
ci <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ci <command> --fields id,title,url
```
...
```bash
ci <command>            # a table, because this is a terminal
ci <command> | wc -l    # JSONL, because this is a pipe
```
