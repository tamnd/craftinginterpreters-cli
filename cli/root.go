// Package cli builds the ci command tree on top of the craftinginterpreters library.
package cli
...
type App struct {
	client *craftinginterpreters.Client
	cfg    craftinginterpreters.Config
...
func Root() *cobra.Command {
	app := &App{cfg: craftinginterpreters.DefaultConfig()}

	root := &cobra.Command{
		Use:   "ci",
		Short: "Browse Crafting Interpreters book chapters",
		Long: `ci reads the Crafting Interpreters book at craftinginterpreters.com.

It lists chapters, searches by title, and returns records as table, JSON,
JSONL, CSV, TSV, or URLs. No API key required — the book is freely available.

ci is an independent tool and is not affiliated with Robert Nystrom or the book.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "base URL for the Crafting Interpreters site")
...
func (a *App) setup() error {
	...
	a.client = craftinginterpreters.NewClient(a.cfg)
	return nil
}
